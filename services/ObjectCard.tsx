...
const ParticleExperience: React.FC<ParticleExperienceProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const initThree = () => {
        ...
        bloomPass.strength = 3.5; // Strong glow for strings
        ...
        // Allocate enough space for many connections (e.g., 500 lines = 1000 vertices)
        ...
        const stringMat = new THREE.LineBasicMaterial({
            ...
            opacity: 1.0, 
            blending: THREE.AdditiveBlending,
            linewidth: 2, // Note: WebGL linewidth is often ignored by browsers, handled via bloom
        });

        const connectorLines = new THREE.LineSegments(connectorGeo, stringMat);
        // Ensure frustum culling doesn't hide lines when vertices move
        ...
        const checkCollisions = (h1: any[], h2: any[]) => {
            ...
            const COLLISION_THRESHOLD = 0.4; // Distance in 3D world units

            TIPS.forEach(tip1 => {
                const p1 = h1[tip1]; // Hand 1 Tip
                
                TIPS.forEach(tip2 => {
                    const p2 = h2[tip2]; // Hand 2 Tip
                    ...
                    if (dist < COLLISION_THRESHOLD) {
                        ...
                        const id = `${tip1}-${tip2}`;
                        const exists = activeConnectionsRef.current.some(c => c.id === id);
        ...
        const animate = () => {
            ...
            if (sceneRef.current) {
                ...
                // Lerp Hand 1
                if (hand1Target && landmarksRef.current.hand1Current) {
                    for (let i = 0; i < hand1Target.length; i++) {
                        landmarksRef.current.hand1Current[i].x = lerp(landmarksRef.current.hand1Current[i].x, hand1Target[i].x, LERP_FACTOR);
                        landmarksRef.current.hand1Current[i].y = lerp(landmarksRef.current.hand1Current[i].y, hand1Target[i].y, LERP_FACTOR);
                        landmarksRef.current.hand1Current[i].z = lerp(landmarksRef.current.hand1Current[i].z, hand1Target[i].z, LERP_FACTOR);
                    }
                ...
                // Lerp Hand 2
                if (hand2Target && landmarksRef.current.hand2Current) {
                    for (let i = 0; i < hand2Target.length; i++) {
                        landmarksRef.current.hand2Current[i].x = lerp(landmarksRef.current.hand2Current[i].x, hand2Target[i].x, LERP_FACTOR);
                        landmarksRef.current.hand2Current[i].y = lerp(landmarksRef.current.hand2Current[i].y, hand2Target[i].y, LERP_FACTOR);
                        landmarksRef.current.hand2Current[i].z = lerp(landmarksRef.current.hand2Current[i].z, hand2Target[i].z, LERP_FACTOR);
                    }
                ...
                if (hand1Target && hand2Target && landmarksRef.current.hand1Current && landmarksRef.current.hand2Current) {
                    ...
                    // Don't clear connections state, but hide lines if hands lost
                ...
                const stringColor = activeColor.clone().lerp(new THREE.Color(0xffffff), 0.6);
                sceneRef.current.stringMaterial.color.copy(stringColor).multiplyScalar(flicker * 1.3);
    ...
    const updateGeometry = (
        ...
        // Clear remaining
        for (let i = lineIdx; i < lineAttr.count; i++) lineAttr.setXYZ(i, 0, 0, 0);
    ...
    const updateActiveConnectors = (
        ...
        // Draw all active connections from the state
        ...
        // Zero out unused buffer
        for (let i = idx; i < lineAttr.count; i++) lineAttr.setXYZ(i, 0, 0, 0);
        lineAttr.needsUpdate = true;
        
        // Ensure geometry calculates bounds properly for culling (though frustumCulled is false, good practice)
    ...
    const predictWebcam = () => {
        ...
        if (videoRef.current.currentTime !== lastVideoTimeRef.current && 
           ...
           // Just update data refs, don't touch scene here
           if (result.landmarks.length > 0) {
               landmarksRef.current.hand1Target = result.landmarks[0];
           } else {
               landmarksRef.current.hand1Target = null;
               // If a hand is lost, do we clear connections? 
               // For "Cat's Cradle" style, maybe we keep them in memory until both hands are lost?
               // Let's keep them for now, it feels more magical if you bring your hand back.
               if (result.landmarks.length === 0) {
                 // activeConnectionsRef.current = []; // Uncomment to auto-clear
               }
           }
