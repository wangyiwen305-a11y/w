...
const MusicVizExperience: React.FC<MusicVizProps> = ({ onBack }) => {
  ...
  const generateShape = (type: string, seedString?: string): Float32Array => {
      ...
      if (type === 'signature' && seedString) {
          ...
          // --- STROBE REIMAGINED: Fractal Torus Knot ---
          let idx = 0;
          ...
          for (let s = 0; s < strands; s++) {
              ...
              for (let i = 0; i < particlesPerStrand; i++) {
                  ...
                  const t = (i / particlesPerStrand) * Math.PI * 2 * 1.5; 
                  const r = torusRadius * (0.8 + 0.4 * Math.cos(q * t)); 
                  const bx = r * Math.cos(p * t);
                  const by = r * Math.sin(p * t);
                  const bz = torusRadius * Math.sin(q * t) * 0.8;
                  const coilFreq = 30.0; 
                  const coilRadius = tubeRadius * (0.5 + 0.5 * Math.sin(t * 10.0)); 
                  const u = t * coilFreq + strandOffset;
                  const ox = coilRadius * Math.cos(u) * Math.cos(p*t); 
                  const oy = coilRadius * Math.sin(u) * Math.sin(p*t);
                  const oz = coilRadius * Math.sin(u + t*5.0);
                  arr[idx*3] = bx + ox;
                  arr[idx*3+1] = by + oy;
                  arr[idx*3+2] = bz + oz;
                  idx++;
          ...
          while (idx < PARTICLE_COUNT) {
              ...
              const theta = Math.random() * Math.PI * 2;
              arr[idx*3] = r * Math.cos(theta);
              arr[idx*3+1] = r * Math.sin(theta);
              arr[idx*3+2] = (Math.random()-0.5) * 0.2;
              idx++;
          }
      }
      return arr;
  ...
  useEffect(() => {
    ...
    // Bloom: Tighter radius for clarity
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(width, height), 1.5, 0.4, 0.85);
    bloomPass.strength = exposure; 
    bloomPass.radius = 0.1; // Reduced radius for sharper look (was 0.2)
    ...
    const animate = () => {
        ...
        // If bass hits a high threshold, trigger flash value
        if (nBass > 0.75) {
            flashValueRef.current = 1.0;
        } else {
            // Decay flash quickly
            flashValueRef.current *= 0.9;
        }
        if (flashValueRef.current < 0.01) flashValueRef.current = 0.0;
        ...
        glitchPass.uniforms.flash.value = flashValueRef.current; // Send flash value
        
        let targetGlitch = isBeat ? nBass * 0.8 : 0.0; 
        targetGlitch += handRef.current.isRippleGesture * 0.8; 
  ...
  return (
    <div className="fixed inset-0 bg-[#020202] text-white font-sans overflow-hidden select-none">
        ...
        <div className="absolute bottom-6 right-6 z-30 flex flex-col items-end pointer-events-none">
            ...
            ></video>
            <div className="text-[10px] text-white/40 mt-1 tracking-widest uppercase font-bold">Input Feed</div>
        </div>
