...
interface Connection {
    ...
    startTime: number; // Timestamp of creation
}

interface TextEntity {
    id: string; 
    mesh: THREE.Mesh;
    wordIndex: number; // Index in the fixed word list
...
// 7 Fixed Words to display simultaneously
const FIXED_WORDS = [
    "INFINITY", "PRISM", "ECHO", "VORTEX", 
    "AURA", "ZENITH", "NEXUS"
];
...
// Helper to detect if a hand is a fist
// Returns true if at least 3 fingers (excluding thumb) are curled close to wrist
const isFist = (landmarks: any[]) => {
    ...
    // Calculate rough hand scale based on palm size
    ...
    for (const tipIdx of tips) {
        ...
        const tdz = wrist.z - tip.z;
        const dist = Math.sqrt(tdx*tdx + tdy*tdy + tdz*tdz);
        
        // Threshold: If tip is close to wrist relative to palm scale
        // 1.8 is a heuristic multiplier derived from testing
        if (dist < scale * 1.8) foldedCount++; 
    }
    
    return foldedCount >= 3; 
};

// Helper to detect if a hand is forming a "Beak" (all 5 fingertips close together)
const isBeak = (landmarks: any[]) => {
    if (!landmarks || landmarks.length < 21) return false;
    ...
    // Check if hand is somewhat extended (thumb tip away from wrist)
    // This helps distinguish "Beak" from "Fist" where fingers are also clumped but near wrist.
    const wristThumbDist = Math.sqrt(
        Math.pow(thumbTip.x - wrist.x, 2) + 
        Math.pow(thumbTip.y - wrist.y, 2) + 
        Math.pow(thumbTip.z - wrist.z, 2)
    );
    if (wristThumbDist < scale * 0.8) return false;

    const tips = [8, 12, 16, 20]; // Index, Middle, Ring, Pinky
    let closeCount = 0;
    
    // Threshold: Tips must be close to Thumb Tip
    const BEAK_THRESHOLD = scale * 0.5; 

    for (const tipIdx of tips) {
        const tip = landmarks[tipIdx];
        const tdx = tip.x - thumbTip.x;
        const tdy = tip.y - thumbTip.y;
        const tdz = tip.z - thumbTip.z;
        const dist = Math.sqrt(tdx*tdx + tdy*tdy + tdz*tdz);
        
        if (dist < BEAK_THRESHOLD) closeCount++; 
    }
    
    return closeCount >= 4; // All fingers touching thumb
};
...
const ParticleExperience: React.FC<ParticleExperienceProps> = ({ onBack }) => {
  ...
  // Text Management - Fixed entities
  const textEntitiesRef = useRef<TextEntity[]>([]);
  const textureCacheRef = useRef<Map<string, THREE.Texture>>(new Map());

  // Performance Throttling
  const lastPredictionTimeRef = useRef(0);

  // Falling Particles State
  const MAX_FALLING_PARTICLES = 3000;
  const isBreakingRef = useRef(false);
  const fallingParticlesRef = useRef<THREE.Points | null>(null);
  const fallingVelocitiesRef = useRef<Float32Array | null>(null);
  ...
  const clearConnections = () => {
      ...
      if (fallingParticlesRef.current) {
          fallingParticlesRef.current.visible = false;
      }
  };

  // Texture Generator
  const getTexture = (text: string): THREE.Texture => {
      if (textureCacheRef.current.has(text)) {
          return textureCacheRef.current.get(text)!;
      }

      const canvas = document.createElement('canvas');
      canvas.width = 512;
      canvas.height = 128; // Rectangular aspect ratio for text
      const ctx = canvas.getContext('2d');
      if (ctx) {
          ctx.clearRect(0, 0, canvas.width, canvas.height); // Transparent background
          
          // Larger font for higher resolution on big scale
          ctx.font = 'bold 90px Arial, sans-serif'; 
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          
          // Light Grey Text - Avoids Bloom Glow, looks flat
  ...
  useEffect(() => {
    ...
    const initThree = () => {
        ...
        fallingGeo.setAttribute('position', new THREE.BufferAttribute(new Float32Array(MAX_FALLING_PARTICLES * 3), 3));
        ...
        const fallingSystem = new THREE.Points(fallingGeo, fallingMat);
        fallingSystem.visible = false;
        // Init velocity buffer
        fallingVelocitiesRef.current = new Float32Array(MAX_FALLING_PARTICLES * 3);
        fallingParticlesRef.current = fallingSystem;

        // Text Group
        const textGroup = new THREE.Group();

        // --- INITIALIZE FIXED TEXT ENTITIES ---
        if (textEntitiesRef.current.length === 0) {
            FIXED_WORDS.forEach((word, index) => {
                const texture = getTexture(word);
                
                // Varied Big Sizes
                const scale = 2.0 + Math.random() * 2.0; 
                const width = 2.0 * (word.length / 5) * scale;
                ...
                const entity: TextEntity = {
                    ...
                    offsetY: (Math.random() - 0.5) * 0.5 * scale, 
                    text: word
                };
                
                textEntitiesRef.current.push(entity);
                textGroup.add(mesh);
            });
        }
        ...
        // Break Strings Logic
        const triggerBreakStrings = (lm1: any[], lm2: any[]) => {
            if (isBreakingRef.current || !fallingParticlesRef.current || !fallingVelocitiesRef.current) return;
            
            isBreakingRef.current = true;
            fallingParticlesRef.current.visible = true;
            
            const positions = fallingParticlesRef.current.geometry.attributes.position.array as Float32Array;
            const velocities = fallingVelocitiesRef.current;
            
            let pIdx = 0;
            const SAMPLES_PER_LINE = 20;

            // Generate particles along active lines
            activeConnectionsRef.current.forEach(conn => {
                const p1Raw = lm1[conn.p1Idx];
                const p2Raw = lm2[conn.p2Idx];

                const p1 = new THREE.Vector3((0.5 - p1Raw.x) * SCALE_X, (0.5 - p1Raw.y) * SCALE_Y, -p1Raw.z * DEPTH_SCALE);
                const p2 = new THREE.Vector3((0.5 - p2Raw.x) * SCALE_X, (0.5 - p2Raw.y) * SCALE_Y, -p2Raw.z * DEPTH_SCALE);

                for(let i=0; i<SAMPLES_PER_LINE; i++) {
                    if (pIdx >= MAX_FALLING_PARTICLES) break;
                    
                    const t = i / SAMPLES_PER_LINE;
                    // Interpolate position + noise
                    positions[pIdx*3] = lerp(p1.x, p2.x, t) + (Math.random()-0.5)*0.1;
                    positions[pIdx*3+1] = lerp(p1.y, p2.y, t) + (Math.random()-0.5)*0.1;
                    positions[pIdx*3+2] = lerp(p1.z, p2.z, t) + (Math.random()-0.5)*0.1;

                    // Initial Velocity: Downward with random spread
                    velocities[pIdx*3] = (Math.random() - 0.5) * 0.05; // X drift
                    velocities[pIdx*3+1] = -Math.random() * 0.05;      // Initial drop
                    velocities[pIdx*3+2] = (Math.random() - 0.5) * 0.05; // Z drift
                    
                    pIdx++;
                }
            });

            // Set draw range to rendered particles
            fallingParticlesRef.current.geometry.setDrawRange(0, pIdx);
            fallingParticlesRef.current.geometry.attributes.position.needsUpdate = true;
            ...
            // Hide Text
            textEntitiesRef.current.forEach(t => t.mesh.visible = false);
        };

        const checkCollisions = (h1: any[], h2: any[]) => {
            // Prevent connection if breaking animation is active
            if (isBreakingRef.current) return;
            // Prevent connection if hands are Fists or Beaks (Gestures that break strings)
            if (isFist(h1) || isFist(h2) || isBeak(h1) || isBeak(h2)) return;
            ...
            TIPS.forEach(tip1 => {
                if (connectedHand1Fingers.has(tip1)) return; 

                TIPS.forEach(tip2 => {
                    if (connectedHand2Fingers.has(tip2)) return; 
                    ...
                    if (dist < COLLISION_THRESHOLD) {
                        const id = `${tip1}-${tip2}`;
                        activeConnectionsRef.current.push({ id, p1Idx: tip1, p2Idx: tip2, startTime: performance.now() });
        ...
        const updateTextEntities = (lm1: any[], lm2: any[], dt: number) => {
            // If breaking or no connections, hide all
            if (isBreakingRef.current || activeConnectionsRef.current.length === 0) {
                textEntitiesRef.current.forEach(entity => {
                    entity.mesh.visible = false;
                });
                return;
            }

            const activeCount = activeConnectionsRef.current.length;
            
            textEntitiesRef.current.forEach((entity, index) => {
                entity.mesh.visible = true;

                const connectionIndex = index % activeCount;
                const conn = activeConnectionsRef.current[connectionIndex];
                ...
                const moveSpeed = entity.speed * 0.3; 
                entity.progress += moveSpeed * entity.direction * dt * 10.0;

                if (entity.progress >= 1.0) {
                    entity.progress = 1.0;
                    entity.direction = -1; 
                } else if (entity.progress <= 0.0) {
                    entity.progress = 0.0;
                    entity.direction = 1; 
                }

                const currentPos = new THREE.Vector3().lerpVectors(p1, p2, entity.progress);
                currentPos.y += entity.offsetY; 
                
                entity.mesh.position.copy(currentPos);
                entity.mesh.lookAt(p2.x, p2.y + entity.offsetY, p2.z);
                entity.mesh.rotateY(Math.PI / 2);
            });
        };
        ...
        const animate = () => {
            ...
            if (sceneRef.current) {
                ...
                // Check Gestures (Break Trigger)
                if (landmarksRef.current.hand1Current && landmarksRef.current.hand2Current) {
                     const h1 = landmarksRef.current.hand1Current;
                     const h2 = landmarksRef.current.hand2Current;
                     
                     // 1. Fist Pull Check
                     const h1Fist = isFist(h1);
                     const h2Fist = isFist(h2);
                     
                     if (h1Fist && h2Fist && activeConnectionsRef.current.length > 0) {
                         // Calculate distance between wrists (index 0)
                         const p1 = h1[0];
                         const p2 = h2[0];
                         const dist = Math.sqrt(Math.pow(p1.x - p2.x, 2) + Math.pow(p1.y - p2.y, 2));

                         // Trigger break only if pulled apart (threshold around 0.2-0.25 of screen width)
                         if (dist > 0.25) {
                            triggerBreakStrings(h1, h2);
                         }
                     }
                     
                     // 2. Beak Check (New) - Fingers gathered together
                     const h1Beak = isBeak(h1);
                     const h2Beak = isBeak(h2);
                     if (h1Beak && h2Beak && activeConnectionsRef.current.length > 0) {
                         triggerBreakStrings(h1, h2);
                     }

                     // 3. Time Limit Check (New) - Natural break after 5 seconds
                     if (activeConnectionsRef.current.length > 0 && !isBreakingRef.current) {
                        const firstConn = activeConnectionsRef.current[0];
                        if (performance.now() - firstConn.startTime > 5000) {
                            triggerBreakStrings(h1, h2);
                        }
                     }
                }

                // 2. Connector Logic (Collision & Rendering)
                if (!isBreakingRef.current && hand1Target && hand2Target && landmarksRef.current.hand1Current && landmarksRef.current.hand2Current) {
                    ...
                    updateTextEntities(landmarksRef.current.hand1Current, landmarksRef.current.hand2Current, delta);
                } else {
                    sceneRef.current.connectorLines.visible = false;
                    textEntitiesRef.current.forEach(t => t.mesh.visible = false);
                }
                
                // 3. Falling Particles Animation
                if (isBreakingRef.current && fallingParticlesRef.current && fallingVelocitiesRef.current) {
                    const positions = fallingParticlesRef.current.geometry.attributes.position.array as Float32Array;
                    const velocities = fallingVelocitiesRef.current;
                    const count = fallingParticlesRef.current.geometry.drawRange.count;
